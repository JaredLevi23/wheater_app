...
class SearchCityDelegate extends SearchDelegate{
  ...
  Widget _buildResults( WheaterBloc wheaterBloc, context){
    ...
    if( query.isEmpty ){
      return const Center(
        child: Text('Ingrese una ciudad para conocer su clima.'),
      );
    }
    ...
    return FutureBuilder(
      ...
      builder: ( _, AsyncSnapshot<List<LocationModel>> snapshot ){
        ...
        if( !snapshot.hasData || snapshot.data!.isEmpty){
          return const Center(
            child: Text('No se encontro la ciudad'),
          );
        }
        ...
        return ListView.separated(
          ...
          itemBuilder: ( _, index ){
            ...
            return ListTile(
              title: Text( location?.name ?? 'Desconocido' ),
              leading: const Icon( Icons.location_city ),
              trailing: const Icon( Icons.arrow_forward_ios_sharp ),
              subtitle: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text( 'Estado: ${location?.state ?? 'Desconocido' }' ),
                  Text( 'Codigo de pais: ${location?.country ?? 'Desconocido' }'),
                ],
              ),
              onTap: () async {
                FocusScope.of(context).unfocus();
                ...
                if( location?.lat != null && location?.lon != null ){
                  ...
                  if( wheater != null ){
                    ...
                    showDialog(context: context, builder: ( context ){
                      return AlertDialog(
                        content: SizedBox(
                          child: Text('No se pudo obtener el clima de ${ location.name }'),
                        ),
                      );
                    });
                  }
